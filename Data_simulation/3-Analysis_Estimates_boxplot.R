...
#estimates_model_1$V2  = factor(estimates_model_1$V2,
...
#   filter(V7 != "216")
estimates_model_1 = estimates_model_1  %>%
  filter(V7 != "216")

aa =estimates_model_1 %>% filter(V2 == alphas.plot[i])
# estimates_model_11 = estimates_model_1 %>% group_by(V2,Parameter,V6,V7,V8) %>%
#   summarise(q1 = quantile(V5,0.25,na.rm=T),
#             q3 = quantile(V5,0.75,na.rm=T),
#             IQR = q3-q1)
for(i in 1:length(alphas.plot) ){
i=2
  fig_box_plot = estimates_model_1 %>% filter(V2 == alphas.plot[i]) %>%

    #ggplot(aes(Method,V2)) +
    ggplot(.,aes(V6,V5)) +
    #geom_boxplot() +
    #geom_boxplot(aes(V6,V5),outlier.shape = NA) +
    stat_summary(fun.data = calc_boxplot_stat , geom="boxplot") +
    #scale_y_continuous(limits = c(estimates_model_11$q1,estimates_model_11$q3))+
    #geom_ridgeline(scale = 2,alpha=0.75) +
    #geom_density_ridges(aes(V2,Method,),scale = 1)  +

    facet_wrap(V3~V7,shrink = T,scales = "free",
               labeller = label_parsed) +
    geom_hline(data =  estimates_model_1 %>% filter(V2 == alphas.plot[i]),
                                                    aes(yintercept = V4))+
    xlab("Method")+
    ylab("Estimates")+
    theme.all
