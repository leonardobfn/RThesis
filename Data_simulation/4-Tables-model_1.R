...
for (AL in al) {
model_1 = estimates_model_1 %>% filter(alpha == AL) %>% arrange(Parameter)
rownames(model_1) = paste0(model_1$X, model_1$Parameter)
model_1 = t(model_1) %>% data.frame()
m1 = model_1[-c(1, 2, 3, 4), seq(1, ncol(model_1), 3)]
m2 = model_1[-c(1, 2, 3, 4), seq(2, ncol(model_1), 3)]
m3 = model_1[-c(1, 2, 3, 4), seq(3, ncol(model_1), 3)]
colnames(m1) = colnames(m2) = colnames(m3) = c(c(1:6))
model_1 = rbind(m1, m2, m3)
...
model_1 = model_1 %>% mutate(Method = meth, Statistics = stats) %>%
  select(Method, Statistics, everything())

  caption_model_1 = paste0("Estimates of parameters to Model 1 and ", "$\\alpha=", AL, "$")

  label_model_1 = paste0("Est_model_1_Alpha", AL)
  table_model_1 = kbl(
    model_1,
    align = ,
    format = "latex"  ,
  ...
  path_model_1 = paste0("Data_simulation/Tables/Table_model_1_Alpha",
                        al.names,
                        ".tex")
  save_kable(table_model_1, path_model_1)
