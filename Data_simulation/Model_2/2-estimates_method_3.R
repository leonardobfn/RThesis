...
snowfall.estimates_Method_3 = function(steps, model, alpha, erro = 10 ^ (-4)) {
  ...
  repeat {
    ...
    if (crit < erro) {
      estimates.aux = list()
      estimates.aux$par <-
        c(emv.beta$par, emv.lambda$par, emv.alpha$par)
      break
    } else{
...
for (alpha. in alphas) {
  ...
  for (steps in idx) {
    ...
    if (par.cov.Gbase$value == 0  ) {
      theta.start = c(start_aux, 0.90)
    } else{
    ...
    if (length(which(is.na(estimates.aux$par) == T)) == 0) {
      if(emv.alpha$value!=0 & emv.beta$value!=0 & emv.lambda$value!=0){
        emv = list()
        emv <- estimates.aux$par
        emv.BETA = list()
        emv.BETA$hessian = emv.beta$hessian
