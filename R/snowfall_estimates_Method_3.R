...
)
{
  ...
  par_names <- c(paste0(colnames(cov_a), "_a"),
                 ...
                 "alpha")
  par_real = read.table("Data_simulation/real_par_model_1.txt")
  beta.real <- par_real[c(1:ncx),1]
  lambda.real <- par_real[c((1 + ncx):(ncx + ncv)),1]%>% as.matrix()
  alpha_ger <- stringr::str_sub(s,6,7) %>% as.numeric()
  ...
  file.sample  = paste0("resultados/data",alpha.value,".txt")
