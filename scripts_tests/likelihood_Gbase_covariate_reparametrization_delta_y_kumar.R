...
#start=c(1,0.15,-0.1,1,1)
start=c(betas,lambdas)
...
for(k in 1:length(N)){
 ...
 write.table(emv,"likelihood_Gbase_covariate_reparametrization_delta_y_kumar.txt",col.names = T)
 emv.results = read.table("likelihood_Gbase_covariate_reparametrization_delta_y_kumar.txt")

boxplot(emv.results$emv.b.3~emv.results$N)
abline(h=betas[3])
means.est <- colMeans(subset(emv.results,N==120)[,-c(1,8)])
means.est <- (means.est[-6])
Beta = means.est[1:3]
