...
log.like.kumar=function(theta){
  ...
  for(t in 2:N[k]){
    a[t] = exp(
      xbeta[t] + ar*(-log(-log(y[t-1])-xbetas[t-1])) +
        ma*(-log(-log(y[t-1])) - log(a[t-1]))
        #ma*(-log(-log(y[t-1])) - mediana[t-1])
    )
    mediana[t] <- log(a[t]) - log(-log(1-.5^(1/b[t])))
  }
...
ar.real=.5
ma.real=.5
start=c(1,0.15,-0.1,1,1,.5,.5)
#start=c(betas,lambdas,ar.real,ma.real)
N=c(1,12,24,36,48,60)*120
#N=c(1)*120
emv = NULL
B=10
...
for(k in 1:length(N)){
  ...
  b.real = 1/delta.real
  a.real = mediana.real=NULL
  y = NULL
  a.real[1] = exp(xbetas[1] )
  mediana.real[1] = log(a.real[1]) - log(-log(1-.5^(1/b.real[1])))
    ...
    for(t in 2:N[k]){
      a.real[t] = exp(
        xbetas[t] + ar.real*(-log(-log(y[t-1])-xbetas[t-1]))+
          ...
          #ma.real*(-log(-log(y[t-1])) - mediana.real[t-1] )
      )
      mediana.real[t] = log(a.real[t]) - log(-log(1-.5^(1/b.real[t])))
