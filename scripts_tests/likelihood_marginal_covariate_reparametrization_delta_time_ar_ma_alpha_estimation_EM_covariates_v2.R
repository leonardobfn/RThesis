...
log.like.marginal_alpha = function(theta, x, w, y,thetas_covariates) {
  ...
  a[1] = exp(xbeta[1])
  for (t in 2:N[k]) {
    a[t] = exp(xbeta[t] + ar * (-log(-log(y[t - 1])) - xbeta[t - 1]))
...
for(k in 1:length(number_of_years)){
  ...
  for(b in 1:B){
    ...
    theta.start <-
      c(betas.real, lambdas.real, ar.real, alpha.real)
    erro = 10^(-4)
    ...
    repeat {
      ...
      par.covariates.start <- theta.start[1:6] # covariates parameter
...
write.table(emv.results,"likelihood_marginal_covariate_reparametrization_delta_time_ar_ma_alpha_estimation_EM_covariates_v2.txt",col.names = T)
emv.results = read.table("likelihood_marginal_covariate_reparametrization_delta_time_ar_ma_alpha_estimation_EM_covariates_v2.txt")

boxplot(emv.results$emv.b.7~emv.results$N.f)
which(is.finite(emv.results$emv.b.7)==F)

emv.results$N.f <- factor(emv.results$N.f)
teste = pivot_longer(emv.results,cols = c(paste0("emv.b.",1:7)))

real.values = data.frame(name=factor(paste0("emv.b.",1:7),
                                     levels = paste0("emv.b.",1:7)),
