...
for (k in 1:length(number_of_years)) {
  ...
  for (b in 1:B) {
    ...
    repeat {
      ...
      par.covariates.start <- estimates.aux$par[-5]
      # Step E-----
...
theta <- estimates %>% filter(V4==144) %>% select(  theta.real ) %>% unlist()
Beta = theta[1:2] # real
lambda = theta[3:4] # real
alpha = theta[5]
xbeta = x %*% Beta
...
al = emv.results %>% filter(V3=="lambda0" & V4=="144")
boxplot(al$V2)
abline(h=-3.0767779)
median(al$V2)
mean(al$V2)
