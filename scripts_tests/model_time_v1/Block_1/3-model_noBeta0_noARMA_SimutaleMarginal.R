...
#0.85512807  0.19572576 -0.08276831 -7.86389466 -7.37369037  0.42049983  0.91158731
N = number_of_years * 12 # length of sample
betas.real = as.matrix(c(0.85512807  ,0.19572576, -0.08276831))
lambdas.real = as.matrix(c(-7.86389466 ,-7.37369037,0.42049983))
alpha.real = 0.91158731
theta.real <- c(betas.real,
                lambdas.real,
                alpha.real)
x <- as.matrix(read.table("scripts_tests/Block_1/cov_a_manaus_m3s1.txt"))
w <- as.matrix(read.table("scripts_tests/Block_1/cov_delta_manaus_m3s1.txt"))
par.names = c(
  paste0("beta",1:3),
  paste0("lambda",1:3),
  "alpha"
...
#k=1
  ...
  for (b in 1:B) {
    ...
    # EM--------
    start_aux <-
      c(0.30835622,0.21981277,-0.04973522,-.1,-.1,-.1 )
...
emv.results = read.table("scripts_tests/Block_1/EMV.txt")
