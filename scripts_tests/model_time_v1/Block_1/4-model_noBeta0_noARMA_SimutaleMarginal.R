...
log.like.kumar = function(theta, x, w, y) {
  ...
  dkumar_log <- extraDistr::dkumar(
    x = y,
    a = a,
    b = b,
    log = T
  )
  l <- dkumar_log[is.finite(dkumar_log)==T]
...
#0.44875154  0.71732701  0.19560292 -0.08309576 -8.18536918 -7.68259363  0.42244745  0.65122490
N = number_of_years * 12 # length of sample
betas.real = as.matrix(c(0.44875154,0.71732701 ,0.19560292,-0.08309576))
lambdas.real = as.matrix(c(-8.18536918 ,-7.68259363 , 0.42244745))
alpha.real = 0.65122490
theta.real <- c(betas.real,
                lambdas.real,
                alpha.real)
x <- as.matrix(read.table("scripts_tests/Block_1/cov_a_manaus_m4s1.txt"))
w <- as.matrix(read.table("scripts_tests/Block_1/cov_delta_manaus_m4s1.txt"))
par.names = c(
  paste0("beta",0:3),
  paste0("lambda",1:3),
  "alpha"
...
for (k in 1:length(number_of_years)) {
  ...
  for (b in 1:B) {
    ...
    # EM--------
    start_aux <-
      c(-1.5984723,0.8009776  , 0.2242229 , -0.0505292  ,-0.1000000,  -0.1000000,  -0.1000000 )
    ...
    repeat {
      ...
      par.covariates.start <-
        theta.start[1:7] # covariates parameter
...
emv.results = read.table("scripts_tests/model_time_v1/Block_1/EMV.txt")
