  ...
  for (b in 1:B) {
    ...
    repeat {
      ...
      par.covariates.start <-
        theta.start[1:4] # covariates parameter
      alpha.start <- theta.start[5] # alpha parameter


      # Step M----

      par.covariates.start.aux = try(optim(par=par.covariates.start,fn = log.like.kumar,method = "BFGS",
      ...
      if (crit < erro) {
        ...
        path_EMV <- "scripts_tests/Block_2/EMV.txt"
...
theta.real <- c(betas.real,
                lambdas.real,
                alpha.real)
emv.results = read.table("scripts_tests/Block_2/EMV.txt")
...
theta <- estimates %>% filter(V4==144) %>% select(  mean ) %>% unlist()
Beta = theta[1:2] # real
lambda = theta[3:4] # real
alpha = theta[5]
xbeta = x %*% Beta
