...
# Estimates--------
  ...
  EMV <- data.frame(city=citys[p],emv.marg=estimates.aux$par,emv.cond=emv,par.names=par.names)
  path_EMV <- "scripts_tests/model_time_v2/Block_3/EMV.txt"
  write.table(EMV, file = path_EMV, append = T, quote = T, col.names = F)
  results[p,] <- emv
  results.mar[p,] <- estimates.aux$par
 # y.data[p,] <- y
}
tic <- tictoc::toc()
