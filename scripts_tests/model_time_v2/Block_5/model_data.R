...
# start values------
  ...
  theta.start <- c(par.cov.Gbase$par,.8)#c(start_aux_betas,-.1,-.1,-.1,-.1,.8) #
  erro = 10 ^ (-4)
  iter = 1
# Estimates-------------
  repeat {
    ...
    # Step E-----
    par.covariates.start <- theta.start[1:(ncx+ncv)]
    alpha.start <- theta.start[-c(1:(ncx+ncv))]
    ...
    # theta.up <- c(par.covariates.up$par)
    ...
    # cat("b=",b,"\n")
    cat("crit=",crit,"\n")
    cat("Estimate=",c(par.covariates.up$par,alpha.up$par),"\n")

    if (crit < erro) {
      emv <- c(par.covariates.up$par,alpha.up$par)
      ...
      iter = iter + 1
      theta.start <- c(par.covariates.up$par,alpha.up$par)
    }
    if(iter==200){
      break
    }
  ...
  # path_hessian <- "scripts_tests/model_time_v2/Block_5/hessian.txt"
  # write.table(hessian, file = path_hessian, append = T, quote = T, col.names = F)
  EMV <- data.frame(city=citys[p],emv.cond=emv,par.names=par.names)
  path_EMV <- "scripts_tests/model_time_v2/Block_5/EMV.txt"
  write.table(EMV, file = path_EMV, append = T, quote = T, col.names = F)
  results[p,] <- emv
