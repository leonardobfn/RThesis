...
# geração pela esperança. Não deu certo. Encontra um alpha maior que 1
# Acredito que com essa geração o modelo entende que as observações são ind.
...
for(t in 2:144){
  ...
  Esp.lambda.aux <- exp(-(Esp.lambda[t])^(1/alpha))
  #Esp.lambda.aux <- exp(-(Esp.lambda[t])^(1))
...
for(t in 2:144){
  ...
  yy[t] = ( 1-Esp.lambda.aux^(1/b[t])  )^(1/a[t])
  #yy[t] = (1-(Esp.lambda.aux)^(1/b[t]))^(1/a[t])
  G.base[t] <- pkumar(y[t],a=a[t],b=b[t],lower.tail = F)
  LAMBDA[t] <- (-log(G.base[t]))
