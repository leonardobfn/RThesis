...
#theta.start <-c(emv1$par[-6],.55) #c(par.cov.Gbase$par,.5)#
#theta.start <-c(emv1$par) #c(par.cov.Gbase$par,.5)#
#theta.start <- c(2.97,0.25,-0.001,-8.36,-.3,0.5)
erro = 10 ^ (-4)
iter = 1
# Estimates-------------
...
for (i in 2:n) {
  ...
  a.ar [i] = exp(xbeta[i] + ar*(  (-log(-log(yy[i-1])))  -(xbeta[i-1] - emv$par[1]) ) )
  #a.ar [i]  = exp(xbeta[i] + ar*(-log(-log(yy[i-1])))  )
  repeat {
    u = runif(N)
    yy[i] = (
      pweibull(
    ...
    if (format(yy[i], scientific = FALSE) != 1) {
      break
    }
  }
}
...
getwd()
write.table(yy,"yy_ar.txt",sep=" ",col.names = F)
