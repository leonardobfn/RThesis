...
theta.start <-c(start_aux,.6) #c(par.cov.Gbase$par,.5)#
theta.start <-c(par.cov.Gbase$par,.5)#c(start_aux,.5) #
theta.start <-c(emv1$par[-6],.50) #c(par.cov.Gbase$par,.5)#
theta.start <-c(emv1$par) #c(par.cov.Gbase$par,.5)#
#theta.start <- emv$par#c(2.97,0.25,-0.001,-8.36,-.3,0.5)
erro = 10 ^ (-4)
iter = 1
# Estimates-------------
...
# geração yt dado yt-1  - geral----------
n=144;N=200
ar = 0#1-alpha
log.at = NULL
u1 = runif(N)
phi.yt = error = matrix(0,n,N)
at1 = a[1]
bt1 = b[1]
u1 = runif(N)
...
for(i in 2:n){
  #i=2
  error[i,] <- -log(-log( ( 1-exp( - (1/b[i])*(-log(u1))^(1/alpha) ) )))
  #log.at[i] <- xbeta[i] + ar * median(phi.yt[i-1,5],na.rm = T)
  log.at[i] <- xbeta[i] + ar * (median(phi.yt[i-1,5],na.rm = T)-xbeta[i-1])
  for(k in 1:N){
    phi.yt[i,k] <- log.at[i] + error[i,k]
  }
}

y.est = exp(-exp(-phi.yt))
plot.ts(y)
lines(rowMeans(y.est),col=2)
yy = rowMeans(y.est)
y.real= y
